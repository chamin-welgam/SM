...
@Component({
  selector : 'SM_VD_DateControlProperty',  
  templateUrl: 'SM_VD_DateControlProperty.component.html',
  providers: [ ],
  styleUrls    : [
    './style/index.less',
    './style/patch.less'
...
export class SM_VD_DateControlPropertyComponent  implements OnInit {
  //variables
  __formNo=0;
  __ID=0;
  __F_NAME="";
  ...
  initForm(){
    ...
    function showData1(_data) {
      if(_data.status=='OK'){
        ...
        var _SP_SM_GET_DATE_CONTROL=281;
        var _data2={
          F_ID: this.__ID };
        var _callBack_fun2 = showData2.bind(this);
        this.callFunction(_SP_SM_GET_DATE_CONTROL, _data2, _callBack_fun2);    
      }else{ //error //todo
    ...
    function showData2(_data) {
      if(_data.status=='OK'){
        this.__F_MIN_DATE = new Date(_data.rows[0].c1);
        this.__F_MAX_DATE = new Date(_data.rows[0].c2);
      }else{ //error //todo
        alert("Error \nwhile creating text control");
      }  
    }    
  ...
  __F_MIN_DATE_Changed(){
    this.UpdateProperty(null,null,null,this.__F_MIN_DATE);
  }
  __F_MAX_DATE_Changed(){
    this.UpdateProperty(null,null,null,null,this.__F_MAX_DATE);
  }

  //functions during the web activities
  UpdateProperty(_F_NAME=null , _F_DISPLAY_NAME=null, _F_ALLOW_NULL=null
      ,_F_MIN_DATE=null, _F_MAX_DATE=null
      ){
    ...
    if(_F_MAX_DATE!=null 
      || _F_MIN_DATE!=null){
      var _funNo2=280;   //SP_SM_UPD_DATE_CONTROL
      var _data2={
        F_ID: this.__ID,
        F_MIN_DATE: _F_MIN_DATE,
        F_MAX_DATE: _F_MAX_DATE
      };
